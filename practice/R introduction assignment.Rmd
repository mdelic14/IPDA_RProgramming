...
library(dplyr)
library(readr)
olympics<-read.csv("D:/Downloads/olympics.csv", skip=1)
...
olympics$Country<-trimws(olympics$Country,"both",whitespace = "[ \t\r\n]")
olympics$Country<-gsub("Â","",olympics$Country)
olympics<-head(olympics,-1)
```
...
```{r}
### olympics %>% filter(olympics$Gold > 0 & olympics$Gold.1 > 0) %>% ### olympics$Country[which.max(abs(olympics$Gold-olympics$Gold.1)/(olympics$Gold.2))]
...
```{r}
result<-mutate(olympics,Points=3*Gold.2+2*Silver.2+Bronze.2)
result<-data.frame(result$Country,result$Points)
```
...
### Reading data
```{r}
census_df<-read.csv("C:/Users/User/Desktop/PracticeR/data/census.csv")
glimpse(census_df)
```
...
```{r}
census<- census_df %>% group_by(STNAME) %>% summarize(n())
names(census)=c("name","counties")
census$name[which.max(census$counties)]
...
```{r}
orderedData <- census_df %>% filter(SUMLEV == "50") %>%
    arrange(STATE, STNAME, desc(CENSUS2010POP))
onlyTop3ByState <- orderedData %>%
...
```{r}
new_census <- filter(census_df, (census_df$SUMLEV == 50))
...
```{r}
census<-subset(census_df,REGION==1 | REGION==2)

census<-census[(grep("Washington*",census$CTYNAME)), ]
census<-subset(census,POPESTIMATE2015>POPESTIMATE2014)
census<-data.frame(census$STNAME,census$CTYNAME)
head(census)
```
